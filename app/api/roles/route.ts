...
export const POST = async (request: Request) => {
  try {
    ...
    const { nom, permissions } = await request.json();
    console.log("📝 Création rôle:", { nom, permissions });
...
export const GET = async (request: Request) => {
  try {
    ...
    // ✅ Vérification permission
    if (!currentUser || !voirPermission(currentUser, "voir_tout_role")) {
      console.log('❌ Accès refusé - Rôle:', currentUser?.role?.nom);
      return NextResponse.json({ 
        message: "Accès refusé. Permission 'voir_tout_role' requise." 
      }, { status: 403 });
    }

    const isAdminUser = estAdmin(currentUser);

    // ✅ Agrégation avec comptage utilisateurs
    const rolesAvecComptes = await Role.aggregate([
      {
        $lookup: {
          from: 'utilisateurs', 
      ...
      },
      {
        $addFields: {
          nombreUtilisateurs: { $size: '$utilisateurs_lies' },
          // ✅ Permissions visibles uniquement pour admin
    ...
    console.log(`✅ ${rolesAvecComptes.length} rôles récupérés`);
